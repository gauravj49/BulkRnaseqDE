...
def main():
    ...
    # Get the differentially expressed smallRNA heatmaps
    get_mirs_enrichment(input_file, get_file_info(output_file)[3], l2fc, padj, base_mean, xclustering, yclustering, rnaseq, column_order, simpleDE)

################ USER DEFINED FUNCTIONS ###################
def get_mirs_enrichment(input_file, output_file, l2fc_param, padj_param, basm_param, xc, yc, rnaseq, column_order, simpleDE):
    ''' Parse the input file and get the log2foldchange for each miRNA '''

    # Get the information in a dictionary
    mir_enrichment = defaultdict(float)
    with open(input_file,'rU') as fg:
        ...
        for line in useful_lines(fg):
            ...
            mir_enrichment[featureid] = de_between
        ...
        if np.array(zmatrix).size:
            print >> sys.stderr, "\t- Drawing the heatmap..."
            if xc:  
                row_method    = 'average'
            else:
                row_method    = None
...
def check_options():
    ...
    parser = argparse.ArgumentParser(add_help=True,
        ...
        ----------------- SAMPLE USAGE ------------------
        - python scripts/draw_mirs_heatmap.py -if=output/filtered_data/deseq2/mirna/results_DEseq2/ACC18mwt_over_ACC4mwt_DE_RESULTS.txt -of=output/filtered_data/deseq2/mirna/heatmaps_DEseq2/ACC18mwt_over_ACC4mwt_DE_heatmaps.txt -xc -yc
        - python scripts/draw_mirs_heatmap.py -if=output/filtered_data/deseq2/mirna/results_DEseq2/Blood18mwt_over_Blood4mwt_DE_RESULTS.txt -of=output/filtered_data/deseq2/mirna/heatmaps_DEseq2/Blood18mwt_over_Blood4mwt_DE_heatmaps.txt -lf=0.25 -pj=0.05 -bm=25
        -------------------------------------------------
        CONTACT: 
        	Gaurav Jain
        	gaurav.jain@dzne.de
